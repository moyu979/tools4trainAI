...
| 脚本 | 说明 |
|------|------|
| **`main.py`** | **[推荐]** 统一输入记录器，同时录制键盘 (K)、鼠标 (M) 和手柄 (G)，输出单个日志文件 |
| `input_recorder_cross_platform.py` | 旧版独立记录器，仅录制键盘和鼠标（使用 pynput） |
| `gamepad_recorder_pygame.py` | 旧版独立记录器，仅录制手柄（使用 pygame） |
| `waterfall_plot.py` | 读取日志，绘制键盘/鼠标/手柄三幅瀑布图并保存为 PNG |
| `key_waterfall_plot.py` | 旧版瀑布图，仅支持键盘+鼠标 |
...
按 **Ctrl+C** 停止录制，日志文件生成在当前目录：`unified_input_YYYYMMDD_HHMMSS.txt`
...
|------|------|--------|---------|
| `K` | 键盘 | pynput | 按键按下/释放 |
| `M` | 鼠标 | pynput | 左右中键点击、移动（降噪过滤）、滚轮滚动 |
| `G` | 手柄 | pygame / SDL | 按钮、方向键、摇杆轴（死区过滤） |

> 设备可用性仅根据库是否存在判断，不扫描物理设备。如果未安装 pygame，手柄录制自动禁用。
...
```
[ISO时间] K PRESS   key=<键名>
[ISO时间] K RELEASE key=<键名>
```

示例：
```
[2026-07-11T12:00:00.123456] K PRESS   key=w
[2026-07-11T12:00:00.234567] K RELEASE key=w
[2026-07-11T12:00:00.345678] K PRESS   key=shift
```
...
```
[ISO时间] M PRESS   button=<左|右|中>
[ISO时间] M RELEASE button=<左|右|中>
...
[ISO时间] G[品牌] AXIS_MOVE   id=<编号> axis=<轴名> val=<数值>
```
...
[2026-07-11T12:02:00.345678] G[Xbox] AXIS_MOVE   id=0 axis=L_Stick_X val=0.4523
...
| 品牌 | 匹配关键词 | 来源 |
|------|-----------|------|
| `Xbox` | 设备名含 "xbox" | Microsoft Xbox 手柄 |
...
**Windows Xbox 手柄映射（部分）：**
```
按钮: 0→A, 1→B, 2→X, 3→Y, 4→LB, 5→RB, 11→D_Up …
轴:   0→L_Stick_X, 1→L_Stick_Y, 4→L_Trigger …
```
...
2. 记录器重新扫描并重建映射表
3. 每次手柄事件还会额外检查映射表是否最新，最多滞后一个事件即恢复
...
```bash
python waterfall_plot.py unified_input_20260711_120000.txt
```

生成三张 PNG 图片：
```
keyboard_waterfall_unified_input_20260711_120000.png
mouse_waterfall_unified_input_20260711_120000.png
gamepad_waterfall_unified_input_20260711_120000.png
```

...
# ============================================================
# 各硬件编码器对应的 GPU 硬件解码器（解码也放到 GPU 上）
# 仅对硬件编码器启用，软件编码器（libx264/libx265）不启用；
# 解码失败时调用方会自动去掉 -hwaccel 回退为软件解码。
# ============================================================
HWACCEL_BY_ENCODER = {
    'hevc_nvenc': 'cuda',
    'h264_nvenc': 'cuda',
...
def build_ffmpeg_command(input_path, output_path, encoder, mode, force=False, retry_8bit=False, hwaccel=True):
    """构建 ffmpeg 编码命令。
    ...
    Args:
        ...
        hwaccel (bool): 是否启用 GPU 硬件解码（仅对硬件编码器有效）。默认为 True。
    ...
    # 硬件编码器：解码也放到 GPU 上（-hwaccel），消除 CPU 解码瓶颈
    hw = HWACCEL_BY_ENCODER.get(encoder)
    if hw and hwaccel:
        command += ['-hwaccel', hw]
...
class VideoProcessor:
    ...
    def _compress_with_retry(self, input_path, output_path):
        ...
        # 硬件解码失败 → 去掉 -hwaccel 重试一次（回退软件解码）
        if HWACCEL_BY_ENCODER.get(self.encoder):
            logger.warning(f"  硬件解码失败，去掉 -hwaccel 重试: {input_path}")
            if output_path.exists():
                output_path.unlink()
            cmd_nohw = build_ffmpeg_command(input_path, output_path, self.encoder, 'archive', hwaccel=False)
            success, comp_time_nohw = self.run_ffmpeg(cmd_nohw, output_path, '压缩')
            comp_time += comp_time_nohw
            if success:
                return True, comp_time, True
    ...
    def _process_train(self, source_file, force=False, source_duration=0.0):
        ...
        # 硬件编码器解码失败 → 去掉 -hwaccel 重试一次（回退软件解码）
        if not success and HWACCEL_BY_ENCODER.get(self.encoder):
            logger.warning(f'硬件解码失败，去掉 -hwaccel 重试: {source_file}')
            if target_file.exists():
                target_file.unlink()
            cmd = build_ffmpeg_command(source_file, target_file, self.encoder, 'train', force=force, hwaccel=False)
            success, elapsed = self.run_ffmpeg(cmd, target_file, '转码')
